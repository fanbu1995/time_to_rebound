...
1. Assume a "reasonable" trajectory of $I$ (e.g., let $I$ simply be some small fraction of $T$) and reduce the system to a 2-dim one; and then we can
    - do NLS
...
2. Adopt a simulation-based approach (a Bayesian approach), which 
    ...
    - technically can deal with an unobserved dimension, **but** it won't be computationally efficient (the latent space is very high-dimensional), and with this specific model there is an identifiability issue (due to the $kI$ and $d_I I$ terms)
